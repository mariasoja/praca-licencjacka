...
\end{equation*}
\item
$F$ jest referencyjnie niezmienna względem $U$ wtedy i tylko wtedy, gdy dla każdego $m_{1}, m_{2} \in M$ z ,$x \in A$ $\textbf{x} \in \domkniecie{A}$ takich, że $m_{1}(\textbf{x})$ oraz $m_{2}(\textbf{x})$ nalezą do $\mathcal{D}$,
\begin{equation*}
m_{1}(\textbf{x})=F(m_{1};\textbf{x}) \iff m_{2}(\textbf{x})=F(m_{2};\textbf{x})
\end{equation*}
\item
...
\begin{equation*}
F(m_{1};\textbf{x})=F(m_{1};\textbf{y}) \iff F(m_{2};\textbf{x})=F(m_{2};\textbf{y}).
\end{equation*}
\end{enumerate} 
\end{definition}
