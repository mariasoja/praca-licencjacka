...
$$
Dniom tygodnia przypiszemy liczby $a=\zbior{0,1,2,3,4,5,6}$
\end{przyklad}
\begin{tw}
Dla zbioru z Przykładu 1, gdzie $M$ jest zbiorem wszystkich przekształceń $A \to a$ oraz $\Phi$ jest klasą funkcji przekształcających $a$ w siebie wtedy taki układ $U$ jest skalą nominalną, w której nie ma porządku.
\end{tw}
\begin{proof}
...
\begin{przyklad}
Rozważmy zbiór $A=\zbior{\textrm{bardzo niski, niski, średni, wysoki, bardzo wysoki}}$ oraz przypiszmy mu zbiór liczb  $a=\zbior{1,2,3,4,5}$. 
\end{przyklad}
\begin{tw}
Dla zbioru z przykładu 2, gdzie $M$ jest zbiorem wszystkich przekształceń $A \to a$ oraz $\Phi$ jest zbiorem ściśle monotonicznych rosnących odwzorowań $a$ w siebie wtedy taki układ $U$ jest skalą porządkową.
\end{tw}
Natomiast w skali porządkowej już jesteśmy w stanie uporządkować nasze dane lecz nie możemy wykonywać żadnych działań.
\begin{przyklad}
Rozważmy zbiór gdzie zmienna jest rokiem urodzenia czyli niech $A=\zbior{1980,1995,1997,2000,2009}$ oraz przypiszmy mu liczby $a=\zbior{1,2,3,4,5}$, $M$ będzie zbiorem wszystkich przekształceń $A \to a$ i $\Phi$ będzie klasą funkcji przekształcających $a$ w siebie.
\end{przyklad}
\begin{tw}
Dla zbioru z przykładu 3, gdzie $M$ jest zbiorem wszystkich przekształceń $A \to a$ oraz $a$ jest zbiorem wszystkich liczb rzeczywistych, $\Phi$ jest zbiorem wszystkich funkcji $\phi$, takich że dla dow. $\beta,\gamma$ gdzie $\beta>0$
\begin{equation*}
\phi(\alpha)=\beta\alpha+\gamma
\end{equation*}
dla wszystkich $\alpha$ z $a$. Wtedy taki ykład $U$ jest skalą interwałową.
\end{tw}
W tej skali już na przykład można określić o ile lat ktoś jest starszy od innej osoby 
\begin{przyklad}
Rozważmy zbiór, gdzie zmienną jest wysokość mierzona w centymetrach czyli niech $A=\zbior{150,155,160,165,170,175}$, $a=\zbior{0,1,2,3,4,5}$, $M$ będzie zbiorem wszystkich przekształceń $A \to a$ i $\Phi$ będzie klasą funkcji przekształcających $a$ w siebie.
\end{przyklad}
\begin{tw}
Dla zbioru z przykładu 4, gdzie $M$ jest zbiorem wszystkich przekształceń $A \to a$ oraz $a$ jest zbiorem wszystkich liczb dodatnich rzeczywistych oraz $\Phi$ jest zbiorem wszystkich przekształceń takich że dla dow. $\beta>0$
\begin{equation*}
\phi(\alpha)=\beta\alpha
\end{equation*}
