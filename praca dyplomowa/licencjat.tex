...
\begin{definition}[Numerical assignment system (NAS){\citep[Str. 101, Def. 1]{adams1965theory}}] 
...
$$ 
\item
$\Phi$ zawiera tożsamościową transformację (identyczność) $a$ w siebie oraz zachodzi
$$
...
\begin{definition}[{\citep[Str. 102, Def. 2]{adams1965theory}}]
...
\end{definition}
\section{Typy skal}
Pojęcie Stevensa na temat typów skal jest jednym z najważniejszych definicji w jego teorii. Skala w teorii tej służy uporządkowaniu, określeniu różnych systemów NAS
\begin{definition}[{\citep[Str. 103, Def. 3]{adams1965theory}}]
...
$$ 
\end{definition}
Domknięcie w pewien szczególny sposób %TODO
\begin{definition}[Rozszerzenie funkcji na skończone ciągi]
Niech $A$ będzie dowolnym niepustym zbiorem. Niech $m:A\to \mathbb{R}$. Wtedy funkcja $\domkniecie{m}:\domkniecie{A}\to \domkniecie{\mathbb{R}}$ dana jest formuła, że
\begin{itemize}
...
\begin{definition}[{\citep[Str. 105, Def. 4.1]{adams1965theory}}]
...
$$
\end{przyklad}
\begin{definition}[{\citep[Str. 105, Def. 4.2]{adams1965theory}}]
...
\begin{definition}[{\citep[Str. 105, Def. 5]{adams1965theory}}]
...
Kolejne twierdzenia ograniczone są do regularnego NAS.
...
\begin{tw}[Warunki równoważne niezmienniczości bezwzględnej {\citep[Str. 107, Tw. 1]{adams1965theory}}]
...
\begin{tw}[Warunki równoważne niezmienniczości odnośnikowej {\citep[Str. 107, Tw. 1]{adams1965theory}}]
...
\end{proof}
\begin{tw}[Warunki równoważne niezmienniczości porównawczej {\citep[Str. 107, Tw. 1]{adams1965theory}}]
Niech $U=\tuple{A,M,a,\Phi}$ będzie regularnym NAS, niech $\mathcal{F}:\mathcal{D}\to \domkniecie{\mathbb{R}}$ i niech $F$ będzie uogólnioną operacją na pomiarach odpowiadającą $\mathcal{F}$. Wtedy
NWSR
\begin{enumerate}
\item
...
\begin{tw}[{\citep[Str. 108, Tw. 2]{adams1965theory}}]
...
Pokażemy, że definicja jest poprawna. Aby tak było wartość funkcji nie może zależeć od wyboru $\overline{\alpha}$. Niech $\overline{\alpha}$, $\overline{\beta} \in \domkniecie{a} \land \mathcal{F}(\overline{\alpha})=z=\mathcal{F}(\overline{\beta})$. Z (3.14) mamy natychmiast, że $\mathcal{F}(\domkniecie{\phi}(\overline{\alpha}))=\mathcal{F}(\domkniecie{\phi}(\overline{\beta)$. To oznacza jednak, że $\Psi(\mathcal{F}(\overline{\alpha}))=\Psi(\mathcal{F}(\overline{\beta}))$. Pozostaje pokazać, że $\Psi_{\phi}$ jest różnowartościowa. Niech $z_1, z_2 \in \mathcal{D}(\Psi_{\phi})$. Zatem istnieją reprezentanci $\overline{\alpha}$, $\overline{\beta}$, że $\mathcal{F}(\overline{\alpha})=z_1$, $\mathcal{F}(\overline{\beta})=z_2$.
\begin{eqnarray*}
...
\begin{definition}[{\citep[Str. 111, Def. 6]{adams1965theory}}]
...
\begin{definition}[{\citep[Str. 111, Def. 7]{adams1965theory}}]
...
\begin{definition}[{\citep[Str. 111, Def. 8]{adams1965theory}}]
...
\begin{definition}[{\citep[Str. 112, Def. 9]{adams1965theory}}]
...
\begin{tw} [{\citep[Str. 112, Tw. 3]{adams1965theory}}]
...
\item
$\implies$
Niech $F$ będzie odnośnikowo niezmiennicza w stosunku do $U$. Dla dow. $m_{1}$, $m_{2} \in M$ $x \in A$ i  $\overline{x} \in \domkniecie{A}$ o ile  $\domkniecie{m_1}(\overline{x})$, $\domkniecie{m_2}(\overline{x})\in \mathcal{D}$ to mamy.
\begin{equation}
 m_{1}(x)=F(m_{1};\overline{x}) \iff m_{2}(x)=F(m_{2};\overline{x}).
\end{equation}
